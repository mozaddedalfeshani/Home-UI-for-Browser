...
import { getChatSessionById, saveChatSession, deleteChatSession } from "@/lib/muradian-ai/db";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!session) return withCorsHeaders(NextResponse.json({ error: "Session not found" }, { status: 404 }), request);
