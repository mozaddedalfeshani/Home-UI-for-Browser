  ...
  try {
    ...
    if (!messages || !Array.isArray(messages)) {
      return withCorsHeaders(
        NextResponse.json({ error: "Messages array required" }, { status: 400 }),
        request,
      );
    }
    ...
    if (!apiKey) {
      ...
      const lastMessages = messages.slice(-5).map((message) => message.content).join(" ");
      const isHard = lastMessages.length > 500 || /code|complex|algorithm|debug|math|solve/i.test(lastMessages);
    ...
    const classificationResponse = await fetch("https://api.deepseek.com/chat/completions", {
      method: "POST",
      headers: {
    ...
    const result = (await classificationResponse.json()) as ClassificationResponse;
