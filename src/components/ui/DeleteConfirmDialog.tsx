...
export function DeleteConfirmDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={cn("sm:max-w-md bg-background/80 backdrop-blur-2xl border-destructive/20 shadow-2xl", className)}>
        <DialogHeader>
        ...
        <div className="py-4">
          <div className="rounded-2xl border border-destructive/10 bg-destructive/5 p-4 flex items-start gap-3">
             ...
             <div>
                <p className="text-sm font-semibold text-destructive/80">Permanent Action</p>
                <p className="text-xs text-muted-foreground/70">This action cannot be undone. Please be careful.</p>
             </div>
          </div>
        </div>
