...
const SettingsMenu = () => {
  ...
  const [authTab, setAuthTab] = useState<
    "login" | "signup" | "verify" | "pull-prompt"
  >("login");
  ...
  const handleSectionChange = (s: SettingsSection) => {
    ...
    setActiveSection(s);
    if (s === "account-profile" && !isProfileLoading && !profileName)
      void loadProfile();
    if (s === "account-memory" && !isMemoryLoading && !memory && !memoryDraft)
      void loadMemory();
  };
  ...
  const APP_NAV = [
    ...
    {
      id: "behavior" as SettingsSection,
      label: "Behavior",
  ...
  const ACCOUNT_NAV = isAuthenticated
    ? [
        {
          id: "account-profile" as SettingsSection,
          label: "My Profile",
          icon: UserIcon,
        },
        {
          id: "account-memory" as SettingsSection,
          label: "Memory",
          icon: AiBrain01Icon,
        },
        {
          id: "account-sync" as SettingsSection,
          label: "Cloud Sync",
  ...
  const renderDesktopContent = () => {
          ...
          <div className="space-y-2">
            {AI_MODELS.map((model) => (
              <button
                ...
                type="button"
                onClick={() =>
                  isLite && toast.info("Please upgrade to Premium")
                }
                className={cn(
    ...
    if (activeSection === "account-login")
      return (
    ...
    if (activeSection === "account-profile")
      return (
    ...
    if (activeSection === "account-memory")
      return (
    ...
    <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2">
                ...
                <div className="flex flex-col w-52 shrink-0 border-r border-border/20 bg-muted/20">
                  ...
                  <nav className="flex-1 overflow-y-auto py-2">
                    ...
                    {!isAuthenticated && (
                      <button
                        type="button"
                        onClick={() => handleSectionChange("account-login")}
                        className={cn(
                          "flex w-full items-center gap-2.5 px-4 py-2.5 text-sm transition-colors",
                          activeSection === "account-login"
                            ? "bg-accent text-foreground font-medium"
                            : "text-muted-foreground hover:bg-accent/60 hover:text-foreground",
                        )}
                      >
                ...
                <div className="flex flex-col flex-1 min-w-0 min-h-0 overflow-hidden">
                  <div className="px-6 py-3.5 border-b border-border/20 shrink-0">
                    <h2 className="text-sm font-semibold text-foreground">
                      {[...APP_NAV, ...ACCOUNT_NAV].find(
                        (n) => n.id === activeSection,
                      )?.label ??
                        (activeSection === "account-sync"
                          ? "Cloud Sync"
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setPendingImportProfile(null);
        }}
      >
