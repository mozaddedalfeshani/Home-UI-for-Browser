...
import { useSettingsStore, DEFAULT_DYNAMIC_WALLPAPERS } from "@/store/settingsStore";
import { useTranslation } from "@/constants/languages";
import { cn } from "@/lib/utils";

export const BackgroundSection = () => {
  const { language, backgroundImage, isDynamicWallpaper, setBackgroundImage, setDynamicWallpaper } = useSettingsStore();
  ...
  return (
    <div className="space-y-2 p-1">
      ...
      <button
        ...
        <div className={cn("relative h-4 w-8 rounded-full transition-colors", isDynamicWallpaper ? "bg-primary" : "bg-muted")}>
          <div className={cn("absolute top-0.5 h-3 w-3 rounded-full bg-background transition-all", isDynamicWallpaper ? "translate-x-4" : "translate-x-0.5")} />
        </div>
      </button>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 px-1 scrollbar-hide h-14 items-center">
        ...
        {DEFAULT_DYNAMIC_WALLPAPERS.map((url, idx) => (
          <button
            ...
            <img src={url} alt={`Wallpaper ${idx}`} className="h-full w-full object-cover" />
          </button>
        ))}
      </div>
    </div>
