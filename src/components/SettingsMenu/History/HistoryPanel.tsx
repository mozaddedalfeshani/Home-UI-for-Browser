...
export function HistoryPanel({ onBack }: HistoryPanelProps) {
  ...
  const entries = [
    ...searchEntries.map((e) => ({ id: `search-${e.id}`, entryId: e.id, type: "search" as const, primary: e.value, secondary: t("searchEntry") })),
    ...tabEntries.map((e) => ({ id: `tab-${e.id}`, entryId: e.id, type: "tab" as const, primary: e.title, secondary: e.url })),
  ];
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      {entries.length > 0 ? (
        <div className="space-y-2 overflow-y-auto flex-1">
          {entries.map((entry) => (
            <div key={entry.id} className="flex items-center gap-3 rounded-xl border border-border/40 bg-muted/10 p-3">
              <div className="p-1.5 rounded-full bg-muted/40 text-muted-foreground shrink-0">
                <HugeiconsIcon icon={entry.type === "search" ? Search01Icon : Globe02Icon} size={13} strokeWidth={2} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-medium text-foreground truncate">{entry.primary}</p>
                <p className="text-[10px] text-muted-foreground truncate">{entry.secondary}</p>
              </div>
              <button
                type="button"
                onClick={() => entry.type === "search" ? removeSearchEntry(entry.entryId) : removeTabEntry(entry.entryId)}
        ...
        <div className="flex flex-col items-center justify-center flex-1 rounded-xl border border-dashed border-border/40 text-center py-8">
          <HugeiconsIcon icon={TimeScheduleIcon} size={28} strokeWidth={1.5} className="text-muted-foreground/40 mb-2" />
          <p className="text-sm font-medium text-foreground">{t("noHistory")}</p>
          <p className="text-xs text-muted-foreground mt-1">{t("historyEmptyDescription")}</p>
        </div>
      )}
    </div>
  );
