        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-all bg-zinc-100 dark:bg-zinc-800/50 hover:bg-zinc-200 dark:hover:bg-zinc-800 border border-transparent focus:outline-none truncate">
              <HugeiconsIcon icon={BotIcon} size={18} className="text-indigo-500 shrink-0" />
          ...
          <DropdownMenuContent align="start" className="w-64 rounded-xl p-2 shadow-xl border-border bg-card">
            <DropdownMenuItem 
              onClick={() => setActiveAgentId(null)}
              className={cn(
                "flex items-center gap-2.5 p-2.5 rounded-lg cursor-pointer transition-colors",
                !activeAgentId ? "bg-primary/10 text-primary" : "hover:bg-muted"
              )}
            >
              <HugeiconsIcon icon={AiChat02Icon} size={18} />
              <div className="flex flex-col">
                <span className="text-sm font-medium">Default Assistant</span>
                <span className="text-[10px] text-muted-foreground">General purpose help</span>
              </div>
            </DropdownMenuItem>
            ...
            {agents.map((agent) => (
              <DropdownMenuItem 
                ...
                className={cn(
                  "flex items-center gap-2.5 p-2.5 rounded-lg cursor-pointer transition-colors",
                  activeAgentId === agent.id ? "bg-primary/10 text-primary" : "hover:bg-muted"
                )}
              >
          ...
          sessions.map((session) => (
            <div 
              ...
              className={cn(
                ...
                currentSessionId === session._id ? "bg-accent text-accent-foreground" : "text-muted-foreground"
              )}
              onClick={() => loadSession(session._id)}
            >
              <HugeiconsIcon icon={AiChat02Icon} size={18} className="shrink-0" />
